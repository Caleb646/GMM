...
urlpatterns = [
    ...
    path("reset-password/", v.ResetPasswordView.as_view(), name="base_user_reset_password"),
    path(
        "reset-password-confirm/<uidb64>/<token>/",
        auth_views.PasswordResetConfirmView.as_view(template_name="auth/reset_password_confirm.html"),
        name="base_user_reset_password_confirm",
    ),
    path(
        "reset-password-complete/",
        auth_views.PasswordResetCompleteView.as_view(template_name="auth/reset_password_complete.html"),
        name="password_reset_complete",
    ),
    path("message-log/<slug:slug>/detailed/", v.MessageLogView.as_view(), name="message_log_detailed"),
    path("message-log/<slug:slug>/resend/", v.MessageLogResendView.as_view(), name="message_log_resend"),
    path(
