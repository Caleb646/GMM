...
# message fragments: # format: (_content, headers)
# [('THIRD MESSAGE\n\nSent from Mail for Windows\n', False),
# ("From: caleb thomas\nSent: Thursday, March 17, 2022 5:27 PM\nTo: caleb thomas\nSubject: RE: Test second user doesn't hit reply all", True),
# ('\nSECOND MESSAGE\n\nSent from Mail for Windows\n', False),
# ("From: caleb thomas\nSent: Thursday, March 17, 2022 5:26 PM\nTo: c.thomas@thomasbuildersonline.com; Caleb Thomas\nSubject: Test second user doesn't hit reply all", True),
# ('\nFIRST MESSAGE\n\xa0\nSent from Mail for Windows', False)]
...
class EmailReplyParser(object):
    ...
    def read(text):
        ...
        """
        # email_message = EmailMessage(text).read()
        # print(
        #     # f"\noriginal text: {text}\n\nmessage fragments:"
        #     f"fragments: {[(frag.content) for frag in email_message.fragments]}"
        # )
...
class EmailMessage(object):
    ...
    def _scan_line(self, line):
        ...
        # print(
        #     f"is quote header: {is_quote_header} is quoted: {is_quoted} is header:"
        #     f" {is_header}\n\nline: {line}"
        # )
        # if not (is_header or is_quoted or is_quote_header):
        #     pattern = re.compile(r"From:(.*?)Sent:(.*?)To:(.*?)Subject:")
        #     nested_header = re.search(pattern, line)
        #     # print(f"nest_header: {nested_header}\n\nline: {line}")
        #     if nested_header:
        #         header_start = nested_header.span(0)[0]
        #         header_end = nested_header.span(0)[1] - (len("Subject:"))
        #         header = line[header_start:header_end]
        #         body_past_header = line[header_end : len(line) + 1]
        #         new_line = header + "\n\n" + body_past_header
        #         line = line[0:header_start]
