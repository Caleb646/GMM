...
class PlainTextParser(BaseBodyParser):
    ...
    def _parse_body(self, data):
        ...
        email_message = parser.BytesParser(
            _class=py_email_message.EmailMessage, policy=policy.default
        ).parsebytes(decoded_data)
...
class GmailParser(BaseParser):
    ...
    def _choose_body_parser(self):
        ...
        query, match, score = u.get_best_match(
            list(self.BODY_PARSERS_XMAILER.keys()),
            [xmail_header],
            lambda x: x.lower().replace(" ", ""),
        )
        chosen_xmailer = self._chosen["headers"]["x_mailer"] = (
            query if score > 60 else "Unknown"
        )
        chosen_xmailer_parser: Dict[str:BaseBodyParser] = self.BODY_PARSERS_XMAILER.get(
            chosen_xmailer, self.BODY_PARSERS_XMAILER["default"]
        )
