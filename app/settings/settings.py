...
if not os.getenv("DEBUG"):  # if DEBUG environment variable is set dont load the .env file
    ...
    path = os.path.join(BASE_DIR, ".env.dev")
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "rfis.context_processors.domain_url",
                "rfis.context_processors.default_timezone",
            ],
        },
    },
]
...
ADMINS_INFO = [
    (ainfo.split(",")[0], ainfo.split(",")[1], ainfo.split(",")[2])
    for ainfo in os.environ["ADMINS"].split("|")
    if ainfo != ""
]
